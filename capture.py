...
def process_image(image):
    ...
    processed_image = cv2.GaussianBlur(processed_image, (5,5), 0) #anti aliasing
...
while(True):
    ...
    edge_screen, original_screen = process_image(screen)
    ...
    last_time = time.time()
    cv2.imshow('window1',cv2.cvtColor(original_screen, cv2.COLOR_BGR2RGB))
